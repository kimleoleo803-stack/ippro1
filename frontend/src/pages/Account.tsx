    ...
    <PageLayout title="ACCOUNT">
      <div className="h-full flex flex-col gap-3 max-w-2xl mx-auto w-full">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          {/* ─── Device Pairing ─── */}
          <div className="glass-card rounded-2xl p-4" data-testid="device-pairing-section">
            <div className="flex items-center gap-2 mb-2">
              <Link2 className="w-4 h-4 text-primary" />
              <p className="text-foreground text-sm font-medium">Device Pairing</p>
            </div>
            <p className="text-muted-foreground text-[11px] mb-3">
              Link a Smart TV or streaming device to your IPTV account
              using a 6-digit PIN — no typing long URLs on your remote.
            </p>
            <div className="flex gap-2">
              ...
              >
                <Smartphone className="w-3.5 h-3.5" /> I'm on the Device
              </button>
              <button
                onClick={() => nav("/pair")}
                className="flex-1 text-[12px] px-3 py-2 rounded-lg glass hover:border-primary/40 text-foreground flex items-center justify-center gap-2 transition-all"
              ...
              >
                <Link2 className="w-3.5 h-3.5" /> I have a PIN
              </button>
            </div>
          </div>
          ...
          <div className="glass-card rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-foreground text-sm font-medium">Playback Mode</p>
            </div>
            <p className="text-muted-foreground text-[11px] mb-3">
              Where streams open when you click a channel.
              <br />
              <span className="text-foreground/70">External</span> opens the
              stream directly in your chosen player app — each viewer streams
              from their own IP, so providers can't ban a shared app IP.
            </p>
            <div className="flex gap-2">
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      toast.success(
                        m === "external"
                          ? "External player mode on"
                          : "Internal player mode on"
                      );
                    }}
                  ...
                  >
                    ...
                    {m === "external" ? "External" : "Internal (built-in)"}
                  </button>
                );
              })}
            </div>
          ...
          <div className="glass-card rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-foreground text-sm font-medium">External Player</p>
            </div>
            <p className="text-muted-foreground text-[11px] mb-3">
              The app that opens when you click a channel in External mode.
              <br />
              <span className="text-foreground/70">VidoPlay</span> is the
              default — install it once, then enjoy one-tap launch on every
              channel.
            </p>
                  ...
                  >
                    <button
                      onClick={() => {
                        ...
                        setExtAppId(app.id);
                        toast.success(`External player → ${app.label}`);
                      }}
                    ...
                    >
                      ...
                      >
                        {app.label}{" "}
                        {app.id === "vidoplay" && (
                          <span className="text-[9px] text-primary ml-1 align-middle">
                            DEFAULT
                          </span>
                        )}
                      </p>
          ...
          <div className="glass-card rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-foreground text-sm font-medium">Player Identity</p>
            </div>
            <p className="text-muted-foreground text-[11px] mb-3">
              The User-Agent header sent with stream &amp; API requests.
              Choose the one your IPTV provider expects.
            </p>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setUa(preset.value);
                      toast.success(`User-Agent → ${preset.label}`);
                    }}
            ...
            <p className="text-muted-foreground text-[10px] mt-2 truncate">
              Sending: <span className="text-foreground/70">{ua}</span>
            </p>
          </div>
          ...
          {profiles.length === 0 && (
            <div className="glass-card rounded-2xl p-8 text-center">
              ...
              <p className="text-muted-foreground text-xs">
                Add an Xtream Codes server to get started.
              </p>
            </div>
          )}
