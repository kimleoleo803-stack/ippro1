...
const Movies = () => {
  ...
  if (!activeProfile) {
    return (
      <PageLayout title={t("movies.title")}>
        <div className="h-full flex items-center justify-center">
          <div className="glass-card rounded-2xl p-6 text-center max-w-sm">
            <p className="text-foreground text-sm mb-3">{t("movies.noActiveServer")}</p>
            {isPaidUser ? (
              <p className="text-muted-foreground text-xs">{t("movies.contactAdmin")}</p>
            ) : (
              <Link to="/account" className="text-primary text-xs underline">{t("movies.addProfile")}</Link>
            )}
          </div>
        </div>
      </PageLayout>
      ...
      <div className="flex flex-col gap-3 h-full">
        <div className="flex gap-2">
          ...
          <div className="glass-card rounded-xl px-3 py-2 flex items-center gap-2 flex-1">
            ...
            <input type="text" placeholder={t("movies.searchPlaceholder")} value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          {filtered.slice(0, 300).map((movie) => (
            ...
            >
              <div className="aspect-[2/3] bg-muted/20 flex items-center justify-center relative overflow-hidden">
                {movie.posterUrl ? (
                  <img src={movie.posterUrl} alt={movie.name} loading="lazy" decoding="async" className="w-full h-full object-cover"
